...
class MultiOrgResolver:
    ...
    async def resolve_org_id(self, org_name: str) -> Optional[int]:
        ...
        result = await self.oracle_port.execute_query(
            "SELECT organization_id FROM hr_all_organization_units WHERE name = :org_name",
            {"org_name": org_name},
        )
    ...
    async def get_org_context(self, org_id: int) -> Optional[MultiOrgContext]:
        ...
        result = await self.oracle_port.execute_query(
            "SELECT set_of_books_id, ledger_id FROM hr_operating_units WHERE organization_id = :org_id",
            {"org_id": org_id},
        )
        if result:
